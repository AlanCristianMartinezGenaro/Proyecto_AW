...
<head>
  ...
  <title>Xochitl Store - Busqueda</title>
...
<body>
 ...
 $conexion = mysqli_connect("localhost", "u492204955_root","root34_tienda_P", "u492204955_tienda_p");
  ...
  <!---                Fin del Navbar              ----->
<br>
  <!---                Inicio Targetas de productos buscados              ----->
  <?php
    $buscar = $_GET["buscar"];
    $query = "SELECT * FROM tb_productos INNER JOIN tb_categorias
    ON tb_productos.id_categoria = tb_categorias.id_categoria where nombre_c LIKE '%$buscar%' OR nombre_p LIKE '%$buscar%' ;";
    $results1 = mysqli_query($conexion, $query);
    $num = $results1->num_rows;
    ?>
  ...
  <!---                Fin Targetas de productos buscados              ----->
  </body>
</html>
